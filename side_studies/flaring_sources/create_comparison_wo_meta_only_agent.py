#!/usr/bin/env python3
"""
Script to create comparison files for Flaring Sources evaluation outputs.

This script loads results from two evaluation systems:
1. OpenAI (direct GPT-5.1)
2. PLLM with full multi-agent system

It creates two output files:
1. flaring_comparison_of_all_models.json - Basic comparison with answers only
2. flaring_comparison_of_all_models_with_catalog_data.json - Comparison with full catalog metadata

Input:
- flaring_openai_results_0_25.json (OpenAI direct analysis)
- flaring_output_results_0_25.json (PLLM multi-agent analysis)
- flaring_sources_extracted.json (source metadata)

Output:
- flaring_comparison_of_all_models.json
- flaring_comparison_of_all_models_with_catalog_data.json
"""
...
PLLM_FULL_FILE = "flaring_output_results_0_25.json"
INPUT_SOURCES_FILE = "flaring_sources_extracted.json"

# Output files
OUTPUT_FILE_1 = "flaring_comparison_of_all_models.json"
OUTPUT_FILE_2 = "flaring_comparison_of_all_models_with_catalog_data.json"
...
def create_comparison_entry(
    ...
    pllm_full_result: Optional[Dict[str, Any]] = None,
    ...
    # Add truncation metadata if available (from OpenAI results)
    if openai_result:
        if openai_result.get("event_list_truncated"):
            entry["openai_event_list_truncated"] = True
            entry["openai_original_event_count"] = openai_result.get("original_event_count")
            entry["openai_truncated_event_count"] = openai_result.get("truncated_event_count")
    
    # Add event analysis metadata if available (from PLLM results)
    if pllm_full_result:
        if "event_analysis" in pllm_full_result:
            entry["pllm_has_event_analysis"] = True
        if "neighbor_analysis" in pllm_full_result:
            entry["pllm_has_neighbor_analysis"] = True
        if "neighbors_count" in pllm_full_result:
            entry["pllm_neighbors_count"] = pllm_full_result.get("neighbors_count")
...
def main():
    ...
    for source_id in sorted(all_source_ids):
        ...
        basic_entry = create_comparison_entry(
            ...
            pllm_full_result=pllm_full_result
        )
    ...
    # Summary statistics
    print("=" * 80)
    print("Summary Statistics - Flaring Sources")
    print("=" * 80)
    ...
    truncated_count = sum(1 for entry in comparison_basic if entry.get("openai_event_list_truncated"))
    if truncated_count > 0:
        print(f"Token limit handling:")
        print(f"  Sources with truncated event lists: {truncated_count}/{sources_with_all_data}")
        print()
