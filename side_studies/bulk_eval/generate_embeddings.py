...
END_INDEX = 100    # Exclusive (e.g., 0 to 5 processes indices 0,1,2,3,4)
...
def process_comparison_item(
    ...
    systems = [
        ("open_ai_only", "OpenAI"),
        ("pllm_meta_analyst_only", "PLLM Meta"),
        ("pllm_full", "PLLM Full")
    ]
...
def main():
    ...
    for idx, item in enumerate(comparison_data):
        try:
            ...
            embeddings_count = sum(1 for v in [result.get("open_ai_only"), result.get("pllm_meta_analyst_only"), result.get("pllm_full")] if v is not None)
            
            if embeddings_count == 3:
                successful += 1
    ...
    print(f"  ✅ Successful (all 3 embeddings): {successful}")
    print(f"  ⚠️  Partial (1-2 embeddings): {partial}")
    ...
    for result in embeddings_results:
        for key in ["open_ai_only", "pllm_meta_analyst_only", "pllm_full"]:
