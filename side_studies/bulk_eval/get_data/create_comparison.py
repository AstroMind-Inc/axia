...
This script loads results from three evaluation systems:
1. OpenAI (direct GPT-5-mini)
2. PLLM with only metadata analyst
3. PLLM with full multi-agent system
...
# Input files
OPENAI_FILE = "outputs/gpt-5.1/openai/combined_results.json"
PLLM_META_ONLY_FILE = "outputs/gpt-5.1/only_meta/combined_results.json"
PLLM_FULL_FILE = "outputs/gpt-5.1/all_models/combined_results.json"
...
def create_comparison_entry(
    ...
    entry = {
        ...
        "pllm_meta_analyst_only": pllm_meta_answer if pllm_meta_answer else "Not available",
...
def main():
    ...
    pllm_meta_results = load_json_file(PLLM_META_ONLY_FILE)
    ...
    print(f"  ✅ Loaded {len(pllm_meta_results)} PLLM meta-only results")
    ...
    pllm_meta_lookup = create_source_lookup(pllm_meta_results)
    ...
    all_source_ids.update(pllm_meta_lookup.keys())
    ...
    for source_id in sorted(all_source_ids):
        ...
        pllm_meta_result = pllm_meta_lookup.get(source_id)
        ...
        pllm_meta_answer = extract_final_answer(pllm_meta_result, "pllm_meta") if pllm_meta_result else None
        ...
        if not (openai_answer or pllm_meta_answer or pllm_full_answer):
            print(f"  ⚠️  Warning: No answers found for source {source_id} ({source_name})")
            sources_missing_data += 1
            continue
        ...
        basic_entry = create_comparison_entry(
            ...
            openai_answer, pllm_meta_answer, pllm_full_answer
        )
    ...
    pllm_meta_coverage = sum(1 for entry in comparison_basic if entry["pllm_meta_analyst_only"] != "Not available")
    ...
    print(f"  PLLM meta-only: {pllm_meta_coverage}/{sources_with_all_data} ({100*pllm_meta_coverage/sources_with_all_data:.1f}%)")
