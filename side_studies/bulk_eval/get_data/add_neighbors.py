...
This script enriches input_sources.json with nearest neighbors for each source.
...
INPUT_FILE = os.path.join(SCRIPT_DIR, "..", "input_sources.json")
OUTPUT_FILE = os.path.join(SCRIPT_DIR, "..", "input_sources_with_neighbors.json")
...
def main():
    ...
    print("=" * 80)
    print("ADD NEAREST NEIGHBORS TO INPUT SOURCES")
    print("=" * 80)
    print()
    ...
    for i, source in enumerate(sources, 1):
        ...
        print(f"[{i}/{len(sources)}] {source_name} (ID: {source_id})")
        ...
        if not pca_vector or not isinstance(pca_vector, list) or len(pca_vector) == 0:
            print(f"   ⚠️  No pca_64d vector available - skipping neighbors")
            no_vector_count += 1
            # Add source without neighbors
            source["neighbors"] = []
    ...
    # Sample neighbor
    if enriched_sources and enriched_sources[0].get("neighbors"):
        print("=" * 80)
        print("SAMPLE NEIGHBOR (from first source)")
        print("=" * 80)
        sample_neighbor = enriched_sources[0]["neighbors"][0]
        print(f"Source: {enriched_sources[0].get('source_name')}")
        print(f"Neighbor: {sample_neighbor.get('source_name')} (score: {sample_neighbor.get('score', 0):.4f})")
        print(f"Fields included:")
