#!/usr/bin/env python3
"""
Bulk Metadata Analysis Script for AstroMind Service
====================================================

This script processes astronomical sources using MetadataAnalyst + Critic agents.
It reads sources from an input JSON file, sends them to the streaming endpoint,
and outputs the results to a JSON file.

Usage:
    python bulk_metadata_analysis.py [--concurrent N]
...
START_INDEX = 50   # Start at this index (0-based, inclusive)
END_INDEX = 300    # End before this index (0-based, exclusive)
# Examples:
...
if START_INDEX is not None and END_INDEX is not None:
    OUTPUT_FILE = f"output_results_{START_INDEX}_{END_INDEX}.json"
elif START_INDEX is not None:
    OUTPUT_FILE = f"output_results_from_{START_INDEX}.json"
elif END_INDEX is not None:
    OUTPUT_FILE = f"output_results_to_{END_INDEX}.json"
else:
    OUTPUT_FILE = "output_results.json"

# Failed sources log file (appended across multiple runs)
FAILED_SOURCES_FILE = "failed_sources.txt"
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description='Bulk analysis of astronomical sources using full multi-agent workflow'
    )
    parser.add_argument(
        '--concurrent',
        type=int,
        default=4,
        help='Number of concurrent requests (default: 4, max: 5)'
    )
