...
def generate_answer(model,
    ...
    for k, v in xray_data.items():
        ...
        xray_batch[k] = t.to(device=device,
...
def get_embedding(model, event_data, global_stats):
    data={}
    data = source_to_xray_tensors(event_data, global_stats)
    # data['per_event'] = per_event
    # data['event_mask'] = event_mask
    # data['meta_token'] = meta_token
    # data['spec_vec'] = spec_vec
    # data['psd_vec'] = psd_vec
    ...
    fused = model.xray_processor(
        xray_batch["per_event"],                # positional 'x'
        event_mask   = xray_batch["event_mask"],
        meta_token   = xray_batch["meta_token"],
        spec_vec     = xray_batch["spec_vec"],
        psd_vec      = xray_batch["psd_vec"],
    )
    return fused.cpu().detach().tolist()[0]


def prune(event_list, T=28800):
    """
    Prune a single event list.

    Steps (same logic as before):
      - convert to numeric Nx2
      - normalize time to start at 0
      - filter energies into [0.5, 7.0] keV if values look like keV, otherwise [500, 7000] eV
      - require recorded span >= T seconds (after energy filtering)
      - clip to [0, T]
      - apply a small random shift within first dt, then re-clip to [0, T]
      - ensure at least 2 events remain

    Returns:
      (pruned_events: np.ndarray | None, errors: List[str])
    """
    # 0) Convert to numeric Nx2 without mutating caller input
    try:
    ...
    # 1) Normalize time to start at zero
    arr = arr.copy()
    t_min = float(np.min(arr[:, 0]))
    arr[:, 0] = arr[:, 0] - t_min  # now earliest time is 0

    # 2) Infer energy unit and filter by window
    e_max = float(np.nanmax(arr[:, 1]))
    if e_max <= 20.0:
        emin, emax = 0.5, 7.0       # keV
        unit_label = "keV"
    else:
        emin, emax = 500.0, 7000.0  # eV
        unit_label = "eV"
    ...
    if arr.size == 0:
        return None, [
            ...
            f"[{emin}, {emax}] {unit_label} (had {n_before} events before filtering)"
        ]

    # 3) Require coverage >= T (use last row like original code path)
    #    Also compute available duration for a clearer error message.
    available_sec = float(np.max(arr[:, 0]))  # since we normalized to 0 earlier
    if arr[-1, 0] < T:
        exp_hours = T / 3600.0
        got_hours = available_sec / 3600.0
        return None, [
    ...
    if arr.shape[0] < 2:
        exp_hours = T / 3600.0
        return None, [
            f"event list is invalid - not enough events in the observed period after clipping to first {T} s (~{exp_hours:.2f} h)",
            f"events remaining: {arr.shape[0]}"
        ]

    # 5) Apply small random shift within first interval, then re-clip
    dt = arr[1, 0] - arr[0, 0]
    if dt > 0:
    ...
    if arr.shape[0] < 2:
        ...
        return None, [
            ...
            f"analysis window: first {T} s (~{exp_hours:.2f} h); events remaining: {arr.shape[0]}"
        ]
